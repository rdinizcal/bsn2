...
class RiskAnalyzer:
    ...
    def emit_alert(self, patient_status):
        ...
        if not self.node.active or patient_status <= 0:
            self.node.publisher_manager.publish_status(StatusContent.FAIL, Task.EMIT)
            return
        ...
        try:
            ...
            self.node.publisher_manager.publish_status(StatusContent.SUCCESS, Task.EMIT)
        except Exception as e:
            self.node.get_logger().error(f"Error emitting alert: {e}")
            self.node.publisher_manager.publish_status(StatusContent.FAIL, Task.EMIT)
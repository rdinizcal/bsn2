...
class StatusMessage(DataMessage):
    ...
    def __init__(
        self, timestamp: int, logical_clock: int, source: str, target: str, content: str
    ):
...
class EnergyStatusMessage(DataMessage):
    ...
    def __init__(
        ...
        super().__init__(
            "EnergyStatus", timestamp, logical_clock, source, target, content
        )
...
class UncertaintyMessage(DataMessage):
    ...
    def __init__(
        ...
        super().__init__(
            "Uncertainty", timestamp, logical_clock, source, target, content
        )
...
class AdaptationMessage(DataMessage):
    ...
    def __init__(
        ...
        super().__init__(
            "Adaptation", timestamp, logical_clock, source, target, content
        )
...
class DataAccess(Node):
    ...
    def __init__(self):
        ...
        self.status: Dict[str, deque] = defaultdict(
    ...
    def _initialize_component_data(self):
        ...
        for component in unique_components:
            ...
            self.get_logger().info(
                f"Initializing component in status and events: {key}"
            )
    ...
    def _setup_ros_interfaces(self):
        ...
        self.persist_sub = self.create_subscription(
            Persist, "persist", self.receive_persist_message, qos_profile
        )
        ...
        try:
            ...
            if os.path.exists(reliability_path):
                with open(reliability_path, "r") as f:
                    ...
                    try:
                        self.reliability_formula = Formula(
                            self.reliability_formula_text
                        )
                        self.get_logger().info(
                            f"Loaded and parsed reliability formula: {self.reliability_formula_text}"
                        )
                    except FormulaError as e:
                        self.get_logger().error(
                            f"Error parsing reliability formula: {e}"
                        )
            ...
            for node_data in nodes_data:
                ...
                if "Goal" in node_type:
                    ...
                    if "T1." in text:  # Leaf tasks
                        ...
                        nodes[node_id] = LeafTask(
                            node_id, text, context, cost, reliability, frequency
                        )
                    else:
    ...
    def receive_persist_message(self, msg: Persist):
        ...
        try:
                ...
                if msg.source != "/engine":
                    ...
                    try:
                        ...
                        if (
                            ...
                            self._persist_energy_status(
                                msg.timestamp, msg.source, msg.target, msg.content
                            )

                            self.get_logger().debug(
                                f"Redirected {msg.source} -> {component_name}, energy: {energy_level}"
                            )

                        else:
                            self.get_logger().warn(
                                f"Invalid energy status format: {msg.content}"
                            )

                    except (ValueError, IndexError) as e:
                        self.get_logger().error(
                            f"Error parsing energy status '{msg.content}': {e}"
                        )
            ...
            self.get_logger().error(
                f"Error processing persist message from {msg.source}: {e}"
            )
            import traceback
        ...
        try:
            ...
            for cost_str in costs:
                if ":" in cost_str:
                    ...
                    self._persist_energy_status(
                        msg.timestamp, component, msg.target, str(cost_float)
                    )
            ...
            if request.query == "reliability_formula":
                ...
                for component_key, status_deque in self.status.items():
                    if status_deque:
                        ...
                        status_strings = [
                            entry[1] for entry in status_entries
                        ]  # Get status part
                        ...
                        component_entry = (
                            f"/{component_key}:{status_str},{reliability:.6f}"
                        )
    ...
    def _calculate_component_reliability(self, component: str) -> str:
        ...
        try:
            ...
            self.get_logger().error(
                f"Error calculating reliability for {component}: {e}  status { self.status}"
            )
    ...
    def _persist_energy_status(
        self, timestamp: int, source: str, target: str, content: str
    ):
        """Persist energy status message"""
        msg = EnergyStatusMessage(
            timestamp, self.logical_clock, source, target, content
        )

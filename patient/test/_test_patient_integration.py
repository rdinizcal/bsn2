...
class TestPatientIntegration:
    ...
    def test_state_distribution(self):
        ...
        self.node.get_logger().info(
            f"Value distribution by state: {dict(state_distribution)}"
        )
        self.node.get_logger().info(
            f"Mean: {statistics.mean(values)}, Std dev: {statistics.stdev(values)}"
        )
    ...
    def test_transition_edge_cases(self):
        ...
        # Log results
        self.node.get_logger().info(
            f"Observed {state_changes} potential state transitions in {SAMPLES} samples"
        )
        self.node.get_logger().info(
            f"Values range: min={min(values):.2f}, max={max(values):.2f}"
        )
